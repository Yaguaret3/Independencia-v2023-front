...
import LitoralSubregiones from './LitoralSubregiones';

const LitoralRegion = () => {

  const [litoralColor, setLitoralColor] = useState('black');
	const litoralOpacity = 0.3;
  const [open, setOpen] = useState(false);

	const handleMouseOverLitoral = () => {
		setLitoralColor('lime');
	}
	const handleMouseOutLitoral = () => {
		setLitoralColor('black');
	}
  const handleOpen = () => {
      setOpen(true);
  };
  ...
  return (
    <>
      <Tooltip title="Litoral">
        <g id="Litoral">
          <path
            style={{
              fill: litoralColor,
              opacity: litoralOpacity
            }}
            d="M918.33,513.15c-1.03-3.78-1.42-7.93-1.67-11.49l-0.03-0.39c-0.38-5.39-0.68-9.65-4.4-13
   ...
   C914.78,531.89,921.44,524.61,918.33,513.15z"
            onMouseOver={handleMouseOverLitoral}
            onMouseOut={handleMouseOutLitoral}
            onClick={handleOpen} />
        </g>
      </Tooltip>
